 ...
 */
//用户搜索结果界面
public class FindAccountResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_show_list);    //加载用户搜索结果界面
        TextView txt_topbar = (TextView) findViewById(R.id.txt_topbar);
        txt_topbar.setText("搜索结果");    //设置标题
        sIP = new ServerIP();
        String ip = sIP.getIp();    //服务器ip地址
        mLayoutManager = new LinearLayoutManager(FindAccountResultActivity.this, LinearLayoutManager.VERTICAL, false);
    ...
    public void getResult(String ip){
        ...
        String murl = "http://" + ip + ":8000/query/find_account/";
        AsynNetUtils.post(murl, content, new AsynNetUtils.Callback() {
            ...
            public void onResponse(String response) {
                ...
                //解析response中的JSON数据获取用户搜索结果的ArrayList
    ...
    public void initview(String account){
        ...
        // 设置布局管理器
        mRecycleView.addItemDecoration(new DividerDecoration(FindAccountResultActivity.this));    //添加item间的分界线
    ...
    public void Jump(int position, String account){
        ...
        intent.setClass(FindAccountResultActivity.this,AccountInformationActivity.class);    //跳转到用户详情界面
    ...
    public void ifNoItem(){
        if (mAdapter.getItemCount() == 0) {
            final AlertDialog.Builder alertDialogBuilder=new AlertDialog.Builder(FindAccountResultActivity.this);
            alertDialogBuilder.setTitle("没有找到符合的用户，换个搜索词吧！")
                    .setIcon(android.R.drawable.ic_dialog_info)
                    .setPositiveButton("确定",  new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            FindAccountResultActivity.this.finish();    //返回上一界面
                        }
                    }).show();
        }
    }
