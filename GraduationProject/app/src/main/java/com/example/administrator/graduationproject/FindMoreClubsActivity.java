 ...
 */
//发现更多社团界面
public class FindMoreClubsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_find_more);    //加载发现更多社团界面
        TextView txt_topbar = (TextView) findViewById(R.id.txt_topbar);
        txt_topbar.setText("更多社团");    //设置标题
        TextView title = (TextView) findViewById(R.id.title);
        title.setText("推荐社团");    //设置标题
        sIP = new ServerIP();
        String ip = sIP.getIp();    //服务器ip地址
        mLayoutManager = new LinearLayoutManager(FindMoreClubsActivity.this, LinearLayoutManager.VERTICAL, false);
    ...
    public void setListerner(final EditText searchText, Button search_button){
        ...
        search_button.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View view) {
                ...
                //先检查搜索内容是否为空
                if(search_txt.equals("")) Toast.makeText(FindMoreClubsActivity.this, "搜索内容不能为空！", Toast.LENGTH_SHORT).show();
    ...
    public void search(String search_txt){
        ...
        intent.setClass(FindMoreClubsActivity.this,FindClubResultActivity.class);    //跳转到社团搜索结果界面
        intent.putExtras(bundle);
        startActivity(intent);
    }

    //从服务器获取推荐社团
    public void getAdviceClubs(String ip){
        ...
        String url = "http://" + ip + ":8000/query/get_advice_clubs_data/";
        AsynNetUtils.post(url, content, new AsynNetUtils.Callback() {
            @Override
            public void onResponse(String response) {
                //重写onResponse(String response)方法，response为服务器的响应字符串，即JSON数据
                //解析response中的JSON数据获取推荐社团的ArrayList
                ArrayList<club_data> alist = analysisArray(response, club_data.class);
                mAdapter = new club_data_adapter(alist);
                initview(account);    //初始化界面
            }
        });
        /*String url = "http://" + ip + ":8000/get_advice_clubs_data/" + account + "/";
    ...
    public void initview(String account){
        ...
        // 设置布局管理器
        mRecycleView.addItemDecoration(new DividerDecoration(FindMoreClubsActivity.this));    //添加item间的分界线
    ...
    public void Jump(int position, String account){
        ...
        intent.setClass(FindMoreClubsActivity.this,ClubInformationActivity.class);    //跳转到社团详情界面
        intent.putExtras(bundle);
        startActivity(intent);
    }
