 ...
 */
//查看我关注的用户界面
public class MyFollowedAccountsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_show_list);    //加载查看我关注的用户界面
        TextView txt_topbar = (TextView) findViewById(R.id.txt_topbar);
        txt_topbar.setText("我关注的用户");    //设置标题
        sIP = new ServerIP();
        String ip = sIP.getIp();    //服务器ip地址
        mLayoutManager = new LinearLayoutManager(MyFollowedAccountsActivity.this, LinearLayoutManager.VERTICAL, false);
        getFollowedAccounts(ip);    //获取我关注的用户
    }

    //获取我关注的用户。参数为：服务器的ip地址
    public void getFollowedAccounts(String ip){
        ...
        String url = "http://" + ip + ":8000/query/get_followed_accounts_data/";
        AsynNetUtils.post(url, content, new AsynNetUtils.Callback() {
            @Override
            public void onResponse(String response) {
                //重写onResponse(String response)方法，response为服务器的响应字符串，即JSON数据
                //解析response中的JSON数据获取已关注的用户的ArrayList
                ArrayList<account_data> flist = analysisArray(response, account_data.class);
                mAdapter = new account_data_adapter(flist);
                initview(account);    //初始化界面
            }
        });
        /*String url = "http://" + ip + ":8000/get_followed_accounts_data/" + account + "/";
    ...
    public void initview(String account){
        ...
        // 设置布局管理器
        mRecycleView.addItemDecoration(new DividerDecoration(MyFollowedAccountsActivity.this));    //添加item间的分界线
    ...
    public void Jump(int position, String account){
        ...
        intent.setClass(MyFollowedAccountsActivity.this,AccountInformationActivity.class);    //跳转到用户详情界面
    ...
    public void ifNoItem(){
        if (mAdapter.getItemCount() == 0) {
            final AlertDialog.Builder alertDialogBuilder=new AlertDialog.Builder(MyFollowedAccountsActivity.this);
            alertDialogBuilder.setTitle("你还没有关注过用户，快去关注吧！")
                    .setIcon(android.R.drawable.ic_dialog_info)
                    .setPositiveButton("确定",  new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            MyFollowedAccountsActivity.this.finish();    //返回上一界面
                        }
                    }).show();
        }
    }
