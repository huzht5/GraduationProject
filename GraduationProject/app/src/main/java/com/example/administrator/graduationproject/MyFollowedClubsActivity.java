 ...
 */
//查看我关注的社团界面
public class MyFollowedClubsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_show_list);    //加载查看我关注的社团界面
        TextView txt_topbar = (TextView) findViewById(R.id.txt_topbar);
        txt_topbar.setText("我关注的社团");    //设置标题
        sIP = new ServerIP();
        String ip = sIP.getIp();    //服务器ip地址
        mLayoutManager = new LinearLayoutManager(MyFollowedClubsActivity.this, LinearLayoutManager.VERTICAL, false);
        getFollowedClubs(ip);    //获取我关注的社团
    }

    //获取我关注的社团。参数为：服务器的ip地址
    public void getFollowedClubs(String ip){
        ...
        /*String url = "http://" + ip + ":8000/get_followed_clubs_data/" + account + "/";
    ...
    public void initview(String account){
        ...
        // 设置布局管理器
        mRecycleView.addItemDecoration(new DividerDecoration(MyFollowedClubsActivity.this));    //添加item间的分界线
    ...
    public void Jump(int position, String account){
        ...
        intent.setClass(MyFollowedClubsActivity.this,ClubInformationActivity.class);    //跳转到社团详情界面
    ...
    public void ifNoItem(){
        if (mAdapter.getItemCount() == 0) {
            final AlertDialog.Builder alertDialogBuilder=new AlertDialog.Builder(MyFollowedClubsActivity.this);
            alertDialogBuilder.setTitle("你还没有关注过社团，快去关注吧！")
                    .setIcon(android.R.drawable.ic_dialog_info)
                    .setPositiveButton("确定",  new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            MyFollowedClubsActivity.this.finish();    //返回上一界面
                        }
                    }).show();
        }
    }
